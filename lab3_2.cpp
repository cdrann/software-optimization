...
__m256d calc_ITERS(int j, __m256d U_prev_vector_value, __m256d U_curr_vector_value, __m256d U_next_vector_value,
	__m256d P_left_upper, __m256d P_upper, __m256d P_left, __m256d P_curr, __m256d lower_vector_pointer_in_row, __m256d upper_vector_pointer_in_row,
	__m256d prev_vector_pointer_in_row) {
	...
	return SUB(ADD(MUL(v_2cnst, U_curr_vector_value), s3), prev_vector_pointer_in_row);
}
...
void calculate_step(double *P, double *U, double *U_old, double *U_new) {
	for (int i = 1; i < Ny - 1; ++i) {
		...
		for (int j = 0; j <= Nx - ITERS; j += STEP * ITERS) {
			...
			for (int k = 0; k < ITERS; k++) {
				*calculated_vector_pointer_in_row = calc_ITERS(j, U_prev_vector_value, U_curr_vector_value, U_next_vector_value,
					P_left_upper, P_upper, P_left, P_curr,
					*lower_vector_pointer_in_row, *upper_vector_pointer_in_row, *prev_vector_pointer_in_row);
