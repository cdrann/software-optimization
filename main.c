...
void calculate_step(const comp_type *U, const comp_type *P, int n, const comp_type *U_old, comp_type *U_new, comp_type *U_max_n) {
    for(int i = 1; i <= Ny - 2; i++) {
        ...
        for(int j = 1; j <= Nx - 2; j++) {
            comp_type f_ij_n = (j == Sx && i == Sy) ?
                               (exp((-1) * (1 / pow(gamma, 2) ) * pow((2 * M_PI * f0 * (n * tau - t0)), 2) )
...
int main() {
    ...
    for(int n = 0; n < Nt; n++) {
        U_max_n = 0;
        calculate_step(U, P, n, U_old, U_new, &U_max_n);
